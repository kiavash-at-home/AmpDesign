...
namespace AmpDesign
{
	...
	public class Form1 : System.Windows.Forms.Form
	{
		...
		//private int gd,gm,radius,radius1;
		private double
		...
		// Hcp, Vcp,ac2r,ac2d,mc2,d2,l1,l2,l,a5,x,r,r1,i,j,k5,c2,l6,c12,ac1d,nn,ac1r,lAB1,lAB,lBC,lBC1,e1,wl,su,su1,dd,b11,c11,e5,f5,g5,g1,c,d,d1,d5,l5,c1,b5,
		// n1,im2,im,hp,vp,all,hp1,vp1,Gtum,ms1,asd1,c31,s51,t51,Hs1,Vs1,ns,xb11,xb12,bc21,gc21,i1,imd,red,md,add,adr,
		// slm,tlm,num,ap1d,xpp1,xpp2,xppp1,xppp2,xpp,rpp,Hp,Vp,
		public static double
			Gpd,Gpmd,rp,hcp,vcp,ml,al1;
		public static bool myTag = false;
		private string 
			st1,st2,st3,st4,st5,st6,stt1,stt2,s32,s33,sabd,ss9;
			/*s,ss,s3,s4,ss5,ss6,s8,s9,s10,s11,sss,asc,ul,
			st7,st8,st9,st10,stt8,stt9,sang,ar,aaa,st11,
			st12,st13,st14,st15,st16,stt3,stt4,cc3,stt5,stt6,stt7,
			mss,ass,str1,str2,str3,str4,str5,str6,str7,str8,str9,str10,str11,
			str12,s12,ssss,st20,str19,str20,sd,s21,s22,s23,s24,s13,
			s14,s15,s16,SEs,SEs1,ss7,ss8,ss10,ss11,ss12,ss13,ss30,ss31,
			ss34,ss35,s19,sds,gg,snl,sls,kel,sk,sre,sim,scp1,scp2,srp,sbb,
			s7,sbc,asc1,ssd,sss6,sst7,us,sy1,sy2,sel,sl8,sl7,sl11,sl12,sz0,
			srb,sgp,sgpm,sds2,sel1ss1,ss2,ss3,ss4;*/
		...
		private void circle(Pen myPen,double xCenter,double yCenter,double radius)
		{
			...
			//float xCorner = (Convert.ToSingle(860-xCenter)-r);
		...
		private void window(Brush myBrush,double a,double b,double c,double d)
		{
			...
			//myGraph.DrawRectangle(Pens.Black,rect);
		...
		private void outtextxy(Brush myBrush,double x,double y,string s,bool txtVertical)
		{
			...
			//Rectangle rect = new Rectangle(Convert.ToInt16(x),Convert.ToInt16(y),Convert.ToInt16(s.Length*Font.Size),Font.Height);
		...
		#region arc
//		private void arc(double xCenter, double yCenter, double StartAngle, double EndAngle,double radius)
//		{
//			arc(Pens.Black,xCenter,yCenter,StartAngle,EndAngle,radius);
//		}
		...
		private void indicating(double m,double n, double Gpd1)
		{
			...
			//window(Brushes.White,1,1,16,11);
			//clrscr();
			outtextxy(1,10,"m=");
			str(m,ref st1);
			outtextxy(30,10,st1);
		...
		private void matching(double m, double n, double c3)
		{
			...
			//outtextxy(205,2,"PLEASE ENTER Z0 OF LINE=");
			//gotoxy(52,1);
...

			{
					...
					"Amplifer Design",MessageBoxButtons.YesNo, MessageBoxIcon.Question)==DialogResult.Yes)
				{
					...
					//This is a problem on lop and other variables shared between input and output
		...
		private void btnOK_Click(object sender, System.EventArgs Eventarg)
		{
...

			{
				...
				if ((k > 1)&&(ds>1))
					...
					//this.txtOutput.AppendText("It is impossible to design an amplifier with this transistor");
				else if ((k > 1)&&(ds<1))
					#region Unconditionally Stable Code
				{
					...
					//smithchart();
					//setcolor(4);
					//circle(Math.Round(h),Math.Round(v),1);
					//circle(Math.Round(h),Math.Round(v),2);
					//outtextxy(Math.Round(h-2),Math.Round(v-12),"S"); //Print "S" on Source point
					...
					{
						//indicating(ms,asd,myTag?Gpd:Gpd1);
						indicating(ms,asd,0);
					...
					#region Potentially unstable Code
				{
					...
					{ /*Don"t Select Source Ref.Cof.*/
						...
						//{
						//setcolor(15);
						//	outtextxy(115,470,"Reselect Load Ref.Cof. For Suitable Source Ref.Cof.");
						//}/*for reselect load ref.cof.*/
						...
						//smithchart();
						//setcolor(15);
						//circle(Math.Round(Hcp),Math.Round(Vcp),Math.Round(rp*220));/*constant gain circle*/
						...
						//outtextxy(1,50,"if (calculated");
						//outtextxy(1,70,"source ref.cof.");
						//outtextxy(1,90,"is suitable");
						//outtextxy(1,110,"press y,else");
						//outtextxy(1,130,"press enter");
						//gotoxy(1,10);
		...
		{
			e.Graphics.DrawImageUnscaled(myImage,0,232);
		}*/
